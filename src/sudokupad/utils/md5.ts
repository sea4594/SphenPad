/** Pure-JS MD5 implementation copied from the captured SudokuPad utilities. */
export function md5Digest(data: string): string {
  ...
  const rhex = (n: number): string => {
    let out = "";
    for (let j = 0; j < 4; j += 1) out += hexChars[(n >> (j * 8 + 4)) & 0x0f] + hexChars[(n >> (j * 8)) & 0x0f];
    return out;
  };
