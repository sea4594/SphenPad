...
function extractCosmetics(scl: any): PuzzleCosmetics {
  ...
  const parseLayerItem = (item: any) => {
    ...
    return {
      ...
      lineCap: item?.lineCap ?? item?.strokeLinecap,
      lineJoin: item?.lineJoin ?? item?.strokeLinejoin,
      dashArray: Array.isArray(item?.dashArray) ? item.dashArray : Array.isArray(item?.strokeDasharray) ? item.strokeDasharray : undefined,
      dashOffset: parseFiniteNumberToken(item?.dashOffset ?? item?.strokeDashoffset),
