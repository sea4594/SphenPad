...
for sn in range(19,9,-1):
    ...
    if report.exists(): print(f'QTR {sn:03d} existing',flush=True); continue
    ...
    print(f'QTR {sn:03d} start',flush=True); t=time.time()
    try:
        ...
        r=json.loads(report.read_text()) if report.exists() else {}; print(f'QTR {sn:03d} end rc={cp.returncode} passed={r.get("passed")} {time.time()-t:.1f}s',flush=True)
    except subprocess.TimeoutExpired as e:
        text=e.stdout or ''; text=text.decode(errors='replace') if isinstance(text,bytes) else text; log.write_text(text+'\nORCHESTRATOR TIMEOUT\n'); print(f'QTR {sn:03d} timeout',flush=True)
print('QTR COMPLETE',flush=True)
